...
${cvText.substring(0, 12000)}`;
  ...
  try {
    ...
    if (!match) return { statusCode: 500, body: JSON.stringify({ error: 'No se encontró JSON en la respuesta.' }) };
    ...
    try {
      ...
      try {
        ...
        return { statusCode: 500, body: JSON.stringify({ error: 'La IA devolvió una respuesta incompleta. Intentá de nuevo.' }) };
      }
    }
